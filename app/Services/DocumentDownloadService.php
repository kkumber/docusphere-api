...
class DocumentDownloadService
{
    ...
    /**
     * Generate PDF from Cloudinary URL and append SIGNED actions only.
     *
     * @param  string  $cloudinaryUrl
     * @param  array   $signatories  // output of buildSignatoriesFromActions
     * @param  string|null $watermarkText
     * @param  array|null  $options
     * @return string
     */
    public function makeSignedPdfFromUrl(string $cloudinaryUrl, array $signatories, ?string $watermarkText = null, ?array $options = null): string
    {
        $options = array_merge([
            'include_footer' => true,
        ...
        $pdf->MultiCell(0, 5, 'This certification page provides a complete record of all authorized signatories who have reviewed and sugbed this document. The original document content remains unaltered and legally binding.', 0, 'L', false);
        $pdf->Ln(10);
     ...
     */
    public function downloadSignedPdfResponse(string $cloudinaryUrl, array $signatories, ?string $watermarkText = null, string $filename = 'signed_document.pdf')
    {
        $pdfBinary = $this->makeSignedPdfFromUrl($cloudinaryUrl, $signatories, $watermarkText);
