...
class DocumentAssignmentController extends Controller
{
    ...
    {
        $userId = auth()->id();
        $priorityOrder = [Status::DOC_ASSIGN_DELAYED, Status::DOC_RETURNED, Status::DOC_ASSIGN_PENDING, Status::DOC_DRAFT_PENDING, Status::DOC_DRAFT_IN_REVIEW];
        ...
        $assignments = DocumentAssignment::with('document', 'status')
            ...
            ->map(function ($documentAssignment) {
                return [
                    ...
                    'status_id' => in_array($documentAssignment->document->status_id, [Status::DOC_COMPLETED, Status::DOC_ARCHIVED, Status::DOC_REJECTED, Status::DOC_DRAFT_APPROVED]) ? $documentAssignment->document->status->id : $documentAssignment->status->id,
        ...
        $drafts = Document::with('status', 'user')
            ...
            ->whereIn('status_id', [Status::DOC_DRAFT_PENDING, Status::DOC_DRAFT_IN_REVIEW, Status::DOC_DRAFT_APPROVED, Status::DOC_REJECTED, Status::DOC_RETURNED, Status::DOC_COMPLETED, Status::DOC_ARCHIVED])
            ->orderBy('created_at', 'desc')
            ->get()
            ->map(fn($d) => [
                'id' => $d->id,
